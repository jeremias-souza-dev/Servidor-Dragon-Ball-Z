...
if(Modules == nil) then
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		if(parameters.premium and not isPlayerPremiumCallback(cid)) then
			npcHandler:say('Somente players premium podem viajar comigo!', cid)
		...
		elseif(not pzLocked and isPlayerPzLocked(cid)) then
			npcHandler:say('Deve-se estar sem pz locked!', cid)
		elseif(not doPlayerRemoveMoney(cid, parameters.cost)) then
			npcHandler:say('You do not have enough money.', cid)
		else
			npcHandler:say('Adeus, volte quando quiser.', cid)
