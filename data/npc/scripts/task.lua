...
npcHandler:say("Me diga o nome da task que voce deseja fazer!", cid)
...
if not tonumber(contagem) then npcHandler:say('Desculpe, mas voce já terminou a task de '..msg..'.', cid) return true end
setPlayerStorageValue(cid, tasktabble[msg].storage_start, 1)
npcHandler:say("Parabéns, agora voce está participando da task de "..msg..", voce precisa matar "..string.sub(((contagem)-1)-tasktabble[msg].count, 2).." "..msg..".", cid)
talkState[talkUser] = 0
else
npcHandler:say('Desculpe, mas voce já faz parte de uma task!', cid)
talkState[talkUser] = 0
end
else
npcHandler:say('Digite o nome correto da task! Para mais informações digite {!task}', cid)
talkState[talkUser] = 1
end
...
if (contagem == -1) then contagem = 1 end
if not tonumber(contagem) then npcHandler:say('Voce só pode receber os itens uma única vez!', cid) return true end
...
npcHandler:say("Obrigado pela sua ajuda! Recompensas:"..(str == "" and "nenhuma" or ""..str.."").." por ter completado a task de "..k..".", cid)
...
npcHandler:say('Desculpe,mas voce só matou '..((contagem)-1)..' de '..v.count..' '..k..'.', cid)
end
end
end
else
npcHandler:say("Voce não está em nenhuma task.", cid)
end
...
local contagem = getPlayerStorageValue(cid, v.storage)
if (contagem == -1) then contagem = 1 end
npcHandler:say('Voce está participando da task de '..k..' e já tem '..((contagem)-1)..' '..k..' mortos, deseja mesmo sair?', cid)
end
end
else
npcHandler:say("Voce não está em nenhuma task.", cid)
